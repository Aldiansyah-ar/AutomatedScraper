...
def plot_counts(title, daily_counts):
    ...
    ax.set_ylabel('Jumlah Berita', fontsize=11)
...
DATA_DIR = "./"  # Ubah jika CSV ada di sub-folder, misal: "./data"
...
if csv_files:
    ...
    if daily_counts is not None and not daily_counts.empty:
        ...
        col1.metric(label="Total Berita", value=int(daily_counts['jumlah_berita'].sum()))
        col2.metric(label="Jumlah Hari Terdata", value=len(daily_counts))
        
        plot_counts(f"Tren Jumlah Berita ({selected_file_name})", daily_counts)
        
        with st.expander("Lihat Data Detail"):
            st.dataframe(daily_counts, use_container_width=True)
    else:
        st.warning("Tidak ada data tanggal yang valid setelah proses cleaning.")
else:
    st.sidebar.warning(f"Tidak ada file .csv ditemukan di folder `{DATA_DIR}`.")